...
describe('JSONTree Class', () => {
  ...
  describe('Class method', () => {
    ...
    it('this.getHEADTarget() of attached HEAD to be master', () => {
      expect(jsonTree.getHEADTarget()).toEqual('master');
    });

    it('this.getHEADTarget() of detached HEAD to be HEAD', () => {
      const tmpRepo = new TempRepository({ detach: true });
      const repoDetach = new JSONTree(tmpRepo.path);
      expect(repoDetach.getHEADTarget()).toEqual('HEAD');
      tmpRepo.clean();
    });
