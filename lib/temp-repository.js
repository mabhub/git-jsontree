...
const tmp      = require('tmp'); // eslint-disable-line import/no-extraneous-dependencies

class TempRepository {
  constructor ({ detach = false } = {}) {
    this.settings = {
      commitCount: 0,
      branches: [
        'master',
    ...
    const commands = [
      new Command(this.repo, 'config --local init.defaultBranch master'),
      new Command(this.repo, 'branch -m master'),
      ...
      new Command(this.repo, this.commitCmd()),
      new Command(this.repo, this.commitCmd()),
      new Command(this.repo, 'tag dummy -a -m "dummy"'),
      new Command(this.repo, this.commitCmd()),
      new Command(this.repo, this.commitCmd()),
      new Command(this.repo, this.commitCmd()),
      ...
      new Command(this.repo, this.commitCmd()),
      new Command(this.repo, `checkout ${detach ? '--detach' : ''}`),
    ];

    commands.forEach(cmd => cmd.execSync());
  }
